...
class BattleshipGame
{
    static void Main(string[] args)
    {
        ...
        while (!gameOver)
        {
                ...
                if (!string.IsNullOrEmpty(input) && input.Length >= 2)
                {
                    ...
                    if (int.TryParse(colString, out int col) && row >= 0 && row < boardSize && col >= 1 && col <= boardSize)
                    {
                        ...
                        else
                        {
                            ...
                            playerTurn = false; // Koniec tury gracza
                        }
            ...
            while (enemyTurn)
            {
                enemyTurn = EnemyTurn(playerBoard);
    ...
    static bool EnemyTurn(char[,] playerBoard)
    {
        Random random = new Random();
        int row, col;
        ...
        while (true)
        {
            ...
            if (playerBoard[row, col] == 'S')
            {
                ...
                return true; // Trafienie - przeciwnik kontynuuje ruch
            }
            else if (playerBoard[row, col] == '~')
            {
                playerBoard[row, col] = '*';
                Console.WriteLine($"Przeciwnik spudłował: {(char)('A' + row)}{col + 1}");
                return false; // Pudło - koniec tury przeciwnika
            }
        }
    }
}